      ...
      <div class="fixed inset-0 overflow-y-auto">
        <div class="flex min-h-full justify-center p-4 text-center items-center sm:p-0">
          <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all xl:max-w-7xl my-8 min-w-[90%] xs:min-w-[80%] sm:min-w-[70%] md:min-w-[50%] lg:min-w-[40%] xl:min-w-[40%] 2xl:min-w-[30%]">
              <div v-show="props.showLoader" class="w-full h-full absolute bg-white z-50 opacity-90">
              ...
              <div v-if="$slots.header" class="text-base py-4 px-6 lg:px-8 bg-gray-50 border-gray-100 border-b text-gray-500">
              ...
              <div class="bg-white px-6 lg:px-8 pt-3 pb-4">
                <slot name="body"></slot>
              </div>
              <div v-if="$slots.footer" class="bg-gray-50 px-6 lg:px-8 py-3 flex-row-reverse flex">
